<template>
  <div class="cart">
    <!-- 购物车图标 -->
    <RouterLink to="/cart" class="curr"> <i class="iconfont icon-cart"><em>{{$store.getters['cart/validTotal']}}</em></i></RouterLink>
    <!-- 购物车弹层 -->
    <div class="layer" v-if="$store.getters['cart/validTotal']>0 && $route.path !== '/cart' " >
      <div class="list">
        <div class="item" v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
          ...
          <i class="iconfont icon-close-new" @click="deleteCart"></i>
        </div>
      </div>
      <div class="foot">
        <div class="total">
...
export default {
  ...
  setup () {
    ...
    const deleteCart = () => {
      store.dispatch('cart/deleteCart').then(() => {
